...
volatile int isUpdated;  // TODO: HK volatile int instead of atomic_t ///Should we use atomic
                         // builtins?
...
struct param_pass *getRcvdParams(struct param_pass *d1) {
    ...
    if (pthread_mutex_trylock(&data1Mutex) != 0)  // Use trylock since this function is called form
                                                  // rt-thread. return immediately with old values
                                                  // if unable to lock
        return d1;
    // pthread_mutex_lock(&data1Mutex); //Priority inversion enabled. Should force completion of
    // other parts and enter into this section.
...
void autoincrCallback(raven_2::raven_automove msg) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        data1.xd[i].x += static_cast<int>(tmpvec[0]);
        data1.xd[i].y += static_cast<int>(tmpvec[1]);
        data1.xd[i].z += static_cast<int>(tmpvec[2]);
...
void publish_ravenstate_ros(struct robot_device *dev, struct param_pass *currParams) {
    ...
    for (int i = 0; i < NUM_MECH; i++) {
        ...
        msg_ravenstate.grasp_d[j] = static_cast<float>(dev->mech[j].ori_d.grasp) / 1000.0;
...
void publish_marker(struct robot_device *device0) {
    ...
    marker1.action =
        visualization_msgs::Marker::ADD;  // Set the marker action.  Options are ADD and DELETE
    ...
    if (draw_L_sphere) {
        ...
        xform.setValue(_ori->R[0][0], _ori->R[0][1], _ori->R[0][2], _ori->R[1][0], _ori->R[1][1],
                       _ori->R[1][2], _ori->R[2][0], _ori->R[2][1], _ori->R[2][2]);
    ...
    if (draw_L_axes) {
        for (int i = 0; i < 3; i++) {
            ...
            axes[i].action = visualization_msgs::Marker::ADD;  // Set the marker action.  Options
                                                               // are ADD and DELETE
