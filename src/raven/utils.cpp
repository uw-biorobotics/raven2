...
*
* 	\brief function that takes an integer and places it in the target short int, reporting
*under/overflow
*
...
*			0 if no problem
*/
int toShort(int value, int16* target) {
    // Overflow
    if (value > SHORT_MAX) {
        *target = SHORT_MAX;
        return SHORT_OVERFLOW;
    } else if (value < SHORT_MIN) {  // Underflow
        *target = SHORT_MIN;
...
/**
*	\fn int loop_over_joints(struct robot_device* device0, struct mechanism*& _mech, struct
*DOF*& _joint, int& mechnum, int& jnum)
*
*	\brief Iterate over all joints of all mechanisms
*
*	\desc To start iteration, call function with _joint == _mech == NULL
*  		Function iterates by incrementing jnum and mnum from zero and returning the
*appropriate mech and joint.
*  		Iteration terminates when function is called with jnum == MAX_DOF-1 and mnum =
*NUM_MECH-1.
*  		In the terminating case, mnum and jnum are not modified.  _joint and _mech are also
...
* 	\param _mech   a pointer points to the mechanism struct, and it is updated at the end of
*the function
* 	\param _joint  a pointer points to the DOF struct, and it is updated at the end of the
*function
* 	\param mechnum integer value represents the mechanism id, and it is updated at the end of
*the function
* 	\param jnum    integer value represents the joint id, and it is updated at the end of the
*function
*
*	\return 0 if reaches last joint index
*			1 otherwise
*/
int loop_over_joints(struct robot_device* device0, struct mechanism*& _mech, struct DOF*& _joint,
                     int& mechnum, int& jnum) {
    // Initialize iterators
    if (_mech == NULL || _joint == NULL) {
        mechnum = 0;
        jnum = 0;
    } else if (mechnum >= (NUM_MECH - 1) && jnum >= (MAX_DOF_PER_MECH - 1)) {  // Terminating cond.
        return 0;
    } else if (jnum >= (MAX_DOF_PER_MECH - 1)) {  // Joint rollover
        mechnum++;
        jnum = 0;
    } else {  // Joint increment
        jnum++;
...
*  		Function iterates by incrementing jnum from zero and returning the appropriate
*joint.
*  		Iteration terminates when function is called with jnum == MAX_DOF-1.
*  		In the terminating case, jnum is not modified.  _joint is also unmodified in that
*case.
*
...
int loop_over_joints(struct mechanism* _mech, struct DOF*& _joint, int& jnum) {
    ...
    if (_joint == NULL) {
        jnum = 0;
    } else if (jnum >= (MAX_DOF_PER_MECH - 1)) {  // Terminating condition
        return 0;
    } else {  // joint rollover
        jnum++;
        if (jnum == NO_CONNECTION) jnum++;
