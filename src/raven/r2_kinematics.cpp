 ...
 * \param d0 - a pointer points to the device struct, equivalent as robot_device struct, see
 * define.h
...
int r2_fwd_kin(struct device *d0, int runlevel) {
    ...
    if ((runlevel != RL_PEDAL_DN) && (runlevel != RL_INIT)) {
        ...
        updateMasterRelativeOrigin(
            d0);  // Update the origin, to which master-side deltas are added.
    }
 ...
 * \param in_j[6] - 6 element array of joint angles ( float j[] = {shoulder, elbow, ins, roll,
 * wrist, grasp} )
 * \param in_arm - Arm type, left / right ( kin.armtype arm = left/right)
 * \param out_xform - a reference of tf::Transform object represents the forward kinematic transfrom
 * from zero frame to endeffector frame of one arm
 * \return: 0 on success, -1 on failure
 *  \ingroup Kinematics
 */
int fwd_kin(double in_j[6], l_r in_arm, tf::Transform &out_xform) {
    ...
    /*
            const static tf::Transform zrot_l( tf::Matrix3x3 (cos(25*d2r),-sin(25*d2r),0,
       sin(25*d2r),cos(25*d2r),0,  0,0,1), tf::Vector3 (0,0,0) );
            const static tf::Transform zrot_r( tf::Matrix3x3 (cos(-25*d2r),-sin(-25*d2r),0,
       sin(-25*d2r),cos(-25*d2r),0,  0,0,1), tf::Vector3 (0,0,0) );
...
/**\fn int getATransform (struct mechanism &in_mch, tf::Transform &out_xform, int frameA, int
 * frameB)
 * \brief Runs the Raven II forward kinematics to determine the desired transform from frame A to
 * frame B
...
int getATransform(struct mechanism &in_mch, tf::Transform &out_xform, int frameA, int frameB) {
    ...
    // rotate to match "tilted" base
    // Needed?  Yes, for transform ^0_xT to get a frame aligned with base (instead of rotated 25
    // degrees to zero angle of shoulder joint)
    /*
            if (frameA == 0)
            {
                    const static tf::Transform zrot_l( tf::Matrix3x3 (cos(25*d2r), -sin(25*d2r), 0,
...
int r2_inv_kin(struct device *d0, int runlevel) {
    ...
    for (int m = 0; m < NUM_MECH; m++) {
        ...
        /*
           ...
           (cos(25*d2r),-sin(25*d2r),0,  sin(25*d2r),cos(25*d2r),0,  0,0,1), tf::Vector3 (0,0,0) );
        ...
        */
        ...
        double thetas_sat[6];
        tf::Transform xf_sat;
        double gangle = static_cast<double>(d0->mech[m].ori_d.grasp) / 1000.0;
 ...
 *
 * See Hawkeye King, Sina Nia Kosari, Blake Hannaford, Ji Ma, 'Kinematic Analysis of the
 *Raven-II(tm) Research Surgical Robot Platform,' University of Washington Electrical Engineering
 *Department Technical Report ,Number 2012-0006, June 29, 2012. (Revised March 2014)
 *
 * \param in_T06 - a btTransfrom obejct, transforms the end effector frame to zero frame
 * \param in_arm - Arm type, left / right ( kin.armtype arm = left/right)
 * \param ik_solution iksol[8] - The 8 solutions:  8 element array of joint angles ( float j[] =
 *{shoulder, elbow, vacant joint, ins,roll, wrist, grasp1, grasp2} )
...
int __attribute__((optimize("0"))) inv_kin(tf::Transform in_T06, l_r in_arm, ik_solution iksol[8]) {
    ...
    for (int i = 0; i < 2; i++) {
        double insertion = 0;
        insertion += p05[4 * i].length();  // Two step process avoids compiler optimization problem.
                                           // (Yeah, right. It was the compiler's problem...)
...
int check_solutions(double *in_thetas, ik_solution *iksol, int &out_idx, double &out_err) {
    ...
    if (minerr > eps) {
        ...
        if (gTime % 100 == 0 && iksol[minidx].arm == dh_left) {
            cout << "failed (err>eps) on j=\t\t(" << in_thetas[0] * r2d << ",\t"
                 << in_thetas[1] * r2d << ",\t" << in_thetas[2] << ",\t" << in_thetas[3] * r2d
                 << ",\t" << in_thetas[4] * r2d << ",\t" << in_thetas[5] * r2d << ")" << endl;
            for (int idx = 0; idx < 8; idx++) {
                ...
                // iksol[idx].th1*r2d << ",\t" << iksol[idx].th2*r2d << ",\t" <<iksol[idx].d3 <<
                // ",\t" <<iksol[idx].th4*r2d << ",\t" <<iksol[idx].th5*r2d << ",\t"
                // <<iksol[idx].th6*r2d << ",\n";
            }
        }
        return -1;
 ...
 * \brief converts the inverse kinematic solution to the thethas (detailes refer to the kinematic
 * report)
