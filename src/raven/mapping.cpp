 ...
 * \brief Transform a position increment and an orientation increment from ITP coordinate frame into
 * local robot zero coordinate frame.
...
void fromITP(struct position *delpos, tf::Quaternion &delrot, int armserial) {
    ...
    const tf::Transform GoldZ25(tf::Matrix3x3(cos(-xRot_rad), -sin(-xRot_rad), 0, sin(-xRot_rad),
                                              cos(-xRot_rad), 0, 0, 0, 1),
                                tf::Vector3(0, 0, 0));
    const tf::Transform GreenZ25(
        tf::Matrix3x3(cos(xRot_rad), -sin(xRot_rad), 0, sin(xRot_rad), cos(xRot_rad), 0, 0, 0, 1),
        tf::Vector3(0, 0, 0));

// const tf::Transform GoldZ25  ( tf::Matrix3x3 (0.906,0.423,0,  -0.423,0.906,0,  0,0,1),
// tf::Vector3 (0,0,0) );
// const tf::Transform GreenZ25( tf::Matrix3x3 (0.906,-0.423,0,  0.423,0.906,0,  0,0,1), tf::Vector3
// (0,0,0) );
