...
//    }
    ...
    float motorPos = (2.0 * PI) * (1.0 / (static_cast<float>(ENC_CNTS_PER_REV))) *
                     (f_enc_val - static_cast<float>(joint->enc_offset));
...
void getStateLPF(struct DOF *joint, style t_style) {
    ...
    if (j % 100) {
        i++;
        if (i % (17) == 0) {
            //        log_msg("joint %i enc_val :  %f    f_enc :  %f", joint->type, joint->enc_val,
            //        f_enc_val);
