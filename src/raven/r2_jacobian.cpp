    ...
    for (int m = 0; m < NUM_MECH; m++) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (i < 3) {
                ...
                // we haven't calculated gravity torques on the tool joints yet - probably
                // unnecessary?
                grav_t = 0;
            }
            ...
            /*
            printf("forces check! \n");
            if((i==0) and (check % 1000 == 0))std::cout<<grav_t<<",
            "<<applied_t<<std::endl;
            */
            // finally, subtract gravity joint torque from applied joint torque to find the
            // non-gravity joint torques
            ...
            j_torque[i] =
                (applied_t - grav_t) * DOF_types[i + offset].TR /
                gearbox;  // t_joint = t_capstan * (torque transfer ratio / Gear box ratio)
        }
...
int r2_jacobian::calc_jacobian(float j_pos[6], tool a_tool, int arm_type) {
    ...
    //************************************************ row 4
    //********************************************
    j_matrix(3, 0) = L - Ca12 * Ca23 * C6 * S5 + Ca12 * C4 * C5 * C6 * Sa23 +
                     Ca12 * Sa23 * S4 * S6 - C2 * Ca23 * C4 * C5 * C6 * Sa12 -
                     C2 * Ca23 * Sa12 * S4 * S6 - C2 * C6 * Sa12 * Sa23 * S5 + C4 * Sa12 * S2 * S6 -
                     C5 * C6 * Sa12 * S2 * S4;
    ...
    //************************************************ row 5
    //********************************************
    j_matrix(4, 0) = L * Ca12 * Ca23 * S5 * S6 - Ca12 * C4 * C5 * Sa23 * S6 +
                     Ca12 * C6 * Sa23 * S4 + C2 * Ca23 * C4 * C5 * Sa12 * S6 -
                     C2 * Ca23 * C6 * Sa12 * S4 + C2 * Sa12 * Sa23 * S5 * S6 + C4 * C6 * Sa12 * S2 +
                     C5 * Sa12 * S2 * S4 * S6;
