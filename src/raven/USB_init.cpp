...
#define BOARD_FILE_STR \
    "brl_usb"  /// Device file. xx is the place holder of the serial number. restricted to 2 digits
               /// for now
...
using namespace std;  // TODO:  change to using-declaration???
 ...
 *  \ingroup IO
 */
int getdir(string dir, vector<string> &files) {  //TODO: check/eliminate non-cost reference
    DIR *dp;
    struct dirent *dirp;
 ...
 * \return 0 on success,
 */
int write_zeros_to_board(int boardid) {  // TODO: change type of boardid to int16, int64, etc???
    ...
    for (int i = 0; i < MAX_DOF_PER_MECH; i++) {
        buffer_out[2 * i + 2] = (char)tmp;         //  TODO:  what's wrong with C-style casting??
...
* \return 0 if no USB board found, USB_INIT_ERROR if error was encountered, or # of boards if
    ...
    for (uint i = 0; i < files.size(); i++) {
        ...
        if ((boardid == GREEN_ARM_SERIAL) || (boardid == GOLD_ARM_SERIAL)) {
            ...
            if (boardid == GREEN_ARM_SERIAL) {
                ...
                log_msg("*** WARNING: USB BOARD #%d NOT CONNECTED TO MECH (update defines?).",
                        boardid);
            }
...
void USBShutdown(void) {
    ...
    for (i = 0; i < boardFile.size(); i++) {
        ...
        { ///  TODO:  pylint disagrees with clang-format here!!
...
int usb_reset_encoders(int boardid) {
    ...
    char buf[bufsize]; //  TODO:  can we just put OUT_LENGTH in buf[OUT_LENGTH] ?? Why not?
